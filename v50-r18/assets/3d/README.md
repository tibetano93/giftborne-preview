...
hero contains 30,968 visible triangles in one skinned shell across seven draw groups.
...
complete seven-pass v17 surface. Each ally starts from the same 30,968-triangle
authored v17 body, physically removes 582 relic-blade and 650 back-mantle
triangles, then deterministically retopologizes that common 29,736-triangle
remainder to one shared 8,000-triangle skinned body. The exact
...
New builds use Blender 5.2.x LTS (5.2.1 preferred) and the workspace
optimizer:
...
pnpm exec node scripts/build-authored-motion.mjs `
  ...
  --output apps/miniapp/public/assets/3d/giftborne-authored-motion-v4.glb
```

Blender 5.2.x LTS creates bounded normalized smart-UV islands across each batched
